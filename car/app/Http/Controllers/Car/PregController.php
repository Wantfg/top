...
class PregController extends Controller
{
    public function index()
    {
        ...
        $in_sale_series_pattern = '/<h4(?: class="toolong")?><a href="https:\/\/www.autohome.com.cn\/(\d+)\/#levelsource=000000000_0&amp;pvareaid=101594">(.*?)(?:<\/a><a href="(?:.*)" target="_blank" class="icon-double11">特卖)?<\/a>(?:<i (?:.*)><\/i>)?<\/h4>/';
        ...
        foreach ($brand_row[0] as $k => $brand_each){
            ...
            foreach ($brand_name_row[0] as $kk => $brand_name_each){
                ...
                //保存厂商
                foreach ($brand_name_factory as $kkk => $item){
